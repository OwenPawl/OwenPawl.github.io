...
async function getevents() {
  ...
  try {
    ...
    sessionStorage.setItem("schedule", JSON.stringify(new CustomEvent("scheduleUpdated", { detail: result.sort((a, b) => new Date(a[3]) - new Date(b[3])).map(item=>[...item.slice(0,3),...item.slice(3,5).map(date=>new Intl.DateTimeFormat("en-US", {timeZone: "America/Los_Angeles",hour: "numeric",minute: "2-digit",hour12: true}).format(new Date(date))),...item.slice(5,9)])})));
